...
/** @brief Construtor for the Middle Earth class 
 *
 * @details Creates a random graph of cities based on paramter values 
 * 
 * @param xsize size of the x-axis in the graph
 * @param ysize size of the y-axis in the graph
 * @param num_cities The number of cities you desire in your graph
 * @param seed used in generating a random number of cities 
 */
MiddleEarth::MiddleEarth (int xsize, int ysize, int num_cities, int seed) {
    ...
    for ( int r = 0; r < num_cities; r++ )
        for ( int c = 0; c < num_cities; c++ ) {
            distances[r*num_cities+c] = sqrt((xpos[c]-xpos[r])*(xpos[c]-xpos[r]) + (ypos[c]-ypos[r])*(ypos[c]-ypos[r]));
        }
...
/** @brief Destructor for the Middle Earth class 
 *
 * @details Sauron, the destructor of Middle-Earth. 
 * 
 */
...
/** @brief prints the cities in the Middle Earth graph to the console
 *
...
/** @brief prints the distance from each city to the console
 *
 * @details prints a tab-separated table of the distances (this can be loaded into Excel or similar)
 * 
 */
...
/** @brief Computes the disstanc between two passed cities 
 *
 * @details This method returns the distance between the two passed cities. If  we assume that the hash table (i.e. the map) is O(1), then this method call is also O(1)Creates a random graph of cities based on paramter values 
 * 
 * @return A float representing the distance between two passed cities
...
/** @ Provides a list of cities to travel to.
 * @details The first city is the original start point as well as the end point.The number of cities passed in does not include this start/end point (so therewill be length+1 entries in the returned vector).
 * 
 * @return A vector containg all the cities to traval to
 * @param length The number of cities in the graph
 */
